...
namespace BlackBox.WareHouse.ControlUnit.AnimaControl
{
    public class PilerOfExit : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            TargetPosition1 = Quaternion.Euler(RotationAngle) * (Varibles.GlobalVariable.WareHouse.transform.localPosition + Varibles.GlobalVariable.BidirectionalPositions[PilerNum, 2, 1]);
            TargetPosition1.y = TargetPosition1.y + Varibles.GlobalVariable.WareHouse.transform.localPosition.y;
            ...
            float tempvalue1 = (Varibles.GlobalVariable.BidirectionalPositions[PilerNum, 2, 1] - Cargo.transform.localPosition).z;
            ...
            float TempValue2_1 = Varibles.GlobalVariable.KPD.HighValues[0] + OBJ1.transform.localPosition.y - (Quaternion.Euler(RotationAngle) * CargoPosition).y + high - ForksSize.y + 0.2f;
            ...
            string name = Varibles.GlobalVariable.WareHouse.name + "/ConveyorGroup/BeltConveyors" + (PilerNum + 1).ToString();
        ...
        void FixedUpdate()
        {
            ...
            if (finish[0] == true && finish[1] == false)
            {
                ...
                if (PilerUpPart.transform.localPosition == UpPartPositions[2])
                {
                    ...
                    Tools.Functions.ChangeColor(Cargo.name, Varibles.StorageBinState.OutStore);//修改该货物对应仓位的状态表示颜色
                    Cargo.GetComponent<Varibles.OperatingState>().state = Varibles.CargoState.Exit;
                }
            }
                ...
                if (PilerUpPart.transform.localPosition == UpPartPositions[6])
                {
                    ...
                    if (PilerFork1.transform.localPosition == Fork1Positions[0])
                    {
                        ...
                        if (PilerForks.transform.localPosition == ForksPositions[0])
                        {
                            ...
                            Varibles.GlobalVariable.BidirectionalConveyorStates[PilerNum, 2, 1] = Varibles.State.On;
                            Varibles.GlobalVariable.Wait[PilerNum] = Varibles.WaitState.WaitExit;
                            Varibles.GlobalVariable.ConveyorQueue[PilerNum].Peek().AddComponent<CargoExit>().enabled = false;
                            Varibles.GlobalVariable.ConveyorQueue[PilerNum].Peek().GetComponent<CargoExit>().Speed = Speed;
                            Varibles.GlobalVariable.ExitQueue[PilerNum].Enqueue(Varibles.GlobalVariable.ConveyorQueue[PilerNum].Peek());
