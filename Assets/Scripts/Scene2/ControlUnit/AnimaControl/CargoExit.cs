...
namespace BlackBox.WareHouse.ControlUnit.AnimaControl
{
    public class CargoExit : MonoBehaviour
    {
        ...
        public int PilerNum;
        private Varibles.CargoMessage CM; //货物信息
        ...
        void Start()
        {
            ...
            CM = this.GetComponent<ShowData.ShowCargoInfo>().Cargomessage;
            RotationAngle = Varibles.GlobalVariable.WareHouse.transform.eulerAngles;
            //Speed = 0.8f;
            int Tempi1 = CM.PositionInfo.HighBayNum; int Tempi2 = (Tempi1 + 1) / 2;
            PilerNums = (Varibles.GlobalVariable.KPD.HighBaysNum + 1) / 2; //Debug.Log(PilerNums);
            PilerNum = Tempi2 - 1;
            //顶升移栽对象
            string LiftTransferName = Varibles.GlobalVariable.WareHouse.name + "/LiftTransferGroup/LiftTransfer" + Tempi2.ToString();
            ...
            string LiftTransferName1 = Varibles.GlobalVariable.WareHouse.name + "/LiftTransferGroup/LiftTransfer" + (PilerNums + 1).ToString();
        ...
        public void BiConveyor1_0(int j)
        {
            ...
            {
                //访问0位置可行情况
                if (Varibles.GlobalVariable.BidirectionalConveyorStates[PilerNum, j, 0] == Varibles.State.Off && BidirectionState[j, 0] == false)
                {
                    Varibles.GlobalVariable.BidirectionalConveyorStates[PilerNum, j, 0] = Varibles.State.On;
                    BidirectionState[j, 0] = true;
                    Varibles.GlobalVariable.BidirectionalConveyorStates[PilerNum, j, 1] = Varibles.State.Off;
                    if (j == 2)
                    {
                ...
                {
                    this.transform.localPosition = Vector3.MoveTowards(this.transform.localPosition, Varibles.GlobalVariable.BidirectionalPositions[PilerNum, j, 0], Speed * Time.deltaTime);
                    if (this.transform.localPosition == Varibles.GlobalVariable.BidirectionalPositions[PilerNum, j, 0])
                    {
        ...
        public void BiConveyor0_1(int j)
        {
            ...
            {
                //访问下一个设备1位置可行情况
                if (Varibles.GlobalVariable.BidirectionalConveyorStates[PilerNum, j - 1, 1] == Varibles.State.Off && BidirectionState[j - 1, 1] == false)
                {
                    Varibles.GlobalVariable.BidirectionalConveyorStates[PilerNum, j - 1, 1] = Varibles.State.On;
                    BidirectionState[j - 1, 1] = true;
                    Varibles.GlobalVariable.BidirectionalConveyorStates[PilerNum, j, 0] = Varibles.State.Off;
                }
                //进行过渡
                if (BidirectionState[j - 1, 1] == true && BidirectionFinish[j - 1, 1] == false)
                {
                    this.transform.localPosition = Vector3.MoveTowards(this.transform.localPosition, Varibles.GlobalVariable.BidirectionalPositions[PilerNum, j - 1, 1], Speed * Time.deltaTime);
                    if (this.transform.localPosition == Varibles.GlobalVariable.BidirectionalPositions[PilerNum, j - 1, 1])
                    {
        ...
        public void LiftTransferExit()
        {
            ...
            {
                //访问输送机可行情况
                if (Varibles.GlobalVariable.LiftTransferStates[PilerNum] == Varibles.State.Off && LiftTransferState[PilerNum, 0] == false)
                {
                    Varibles.GlobalVariable.LiftTransferStates[PilerNum] = Varibles.State.On;
                ...
                if (LiftTransferState[PilerNum, 0] == true && LiftTransferFinish[PilerNum, 2] == false)
                {
                    ...
                    if (LiftTransfer.GetComponent<LiftTransferMove>().Finish1 == true)
                    {
                        ...
                        Varibles.GlobalVariable.BidirectionalConveyorStates[PilerNum, 0, 0] = Varibles.State.Off;
                    }
                }
                //货物过渡到顶升移载机上
                if (LiftTransferFinish[PilerNum, 2] == true && LiftTransferFinish[PilerNum, 1] == false)
                {
                    this.transform.localPosition = Vector3.MoveTowards(this.transform.localPosition, Varibles.GlobalVariable.LiftTransferPositions[PilerNum, 1], Speed * Time.deltaTime);
                    if (this.transform.localPosition == Varibles.GlobalVariable.LiftTransferPositions[PilerNum, 1])
                    {
                ...
                if (LiftTransferFinish[PilerNum, 1] == true && LiftTransferFinish[PilerNum, 0] == false)
                {
                    ...
                    if (LiftTransfer.GetComponent<LiftTransferMove>().Finish2 == true)
                    {
                        ...
                        this.transform.parent = Varibles.GlobalVariable.WareHouse.transform;
                        DestroyImmediate(LiftTransfer.GetComponent<LiftTransferMove>());
                        KeyStep = PilerNum;
                        if (Varibles.GlobalVariable.BidirectionalConveyorStates[PilerNum, 0, 0] == Varibles.State.Off && Varibles.GlobalVariable.BidirectionalConveyorStates[PilerNum, 0, 1] == Varibles.State.Off
                            && Varibles.GlobalVariable.BidirectionalConveyorStates[PilerNum, 1, 0] == Varibles.State.Off && Varibles.GlobalVariable.BidirectionalConveyorStates[PilerNum, 1, 1] == Varibles.State.Off
                            && Varibles.GlobalVariable.BidirectionalConveyorStates[PilerNum, 2, 0] == Varibles.State.Off && Varibles.GlobalVariable.BidirectionalConveyorStates[PilerNum, 1, 1] == Varibles.State.Off)
                        {
                            if (Varibles.GlobalVariable.ConveyorQueue[PilerNum].Count == 0
                                || (Varibles.GlobalVariable.ConveyorQueue[PilerNum].Count > 0 && Varibles.GlobalVariable.ConveyorQueue[PilerNum].Peek().GetComponent<Varibles.OperatingState>().state == Varibles.CargoState.Enter))
                            {
                                Varibles.GlobalVariable.ConveyorDirections[PilerNum] = Varibles.Direction.Enter;
                            }
                        }
                    }
                }
        ...
        public void UniConveyor0_1(int i)
        {
            ...
            {
                //访问1位置可行情况
                if (Varibles.GlobalVariable.UnidirectionalConveyorStates[i, 1] == Varibles.State.Off && UnidirectionState[i, 1] == false)
                {
                    Varibles.GlobalVariable.UnidirectionalConveyorStates[i, 1] = Varibles.State.On;
                    UnidirectionState[i, 1] = true;
                    Varibles.GlobalVariable.UnidirectionalConveyorStates[i, 0] = Varibles.State.Off;
                }
                //0位置移动到1位置
                if (UnidirectionState[i, 1] == true && UnidirectionFinish[i, 1] == false)
                {
                    this.transform.localPosition = Vector3.MoveTowards(this.transform.localPosition, Varibles.GlobalVariable.UnidirectionalPositions[i, 1], Speed * Time.deltaTime);
                    if (this.transform.localPosition == Varibles.GlobalVariable.UnidirectionalPositions[i, 1])
                    {
                        UnidirectionFinish[i, 1] = true;
                        if (i == (PilerNums + 3))
                        {
                            AllRight = true;
                            Varibles.GlobalVariable.UnidirectionalConveyorStates[KeyStep, 1] = Varibles.State.Off;
                            ...
                            Varibles.GlobalVariable.ExitQueue[PilerNum].Dequeue();
        ...
        public void UniConveyor2LiftTransfer(int i)
        {
            ...
            {
                //访问LiftTransfer可通行情况
                if (Varibles.GlobalVariable.LiftTransferStates[i] == Varibles.State.Off && LiftTransferState[i, 0] == false)
                {
                    Varibles.GlobalVariable.LiftTransferStates[i] = Varibles.State.On;
                    LiftTransferState[i, 0] = true;
                    Varibles.GlobalVariable.UnidirectionalConveyorStates[i, 1] = Varibles.State.Off;
                }
                //UniConveyor1位置过渡到LIftTransfer
                if (LiftTransferState[i, 0] == true && LiftTransferFinish[i, 0] == false)
                {
                    this.transform.localPosition = Vector3.MoveTowards(this.transform.localPosition, Varibles.GlobalVariable.LiftTransferPositions[i, 0], Speed * Time.deltaTime);
                    if (this.transform.localPosition == Varibles.GlobalVariable.LiftTransferPositions[i, 0])
                    {
        ...
        public void LiftTransfer2UniConveyor(int i)
        {
            if (LiftTransferFinish[i, 0] == true && UnidirectionFinish[i + 1, 0] == false)
            {
                ...
                if (Varibles.GlobalVariable.UnidirectionalConveyorStates[i + 1, 0] == Varibles.State.Off && UnidirectionState[i + 1, 0] == false)
                {
                    Varibles.GlobalVariable.UnidirectionalConveyorStates[i + 1, 0] = Varibles.State.On;
                    UnidirectionState[i + 1, 0] = true;
                }
                //LiftTransfer过渡到UniConveyor0位置
                if (UnidirectionState[i + 1, 0] == true && UnidirectionFinish[i + 1, 0] == false)
                {
                    this.transform.localPosition = Vector3.MoveTowards(this.transform.localPosition, Varibles.GlobalVariable.UnidirectionalPositions[i + 1, 0], Speed * Time.deltaTime);
                    if (this.transform.localPosition == Varibles.GlobalVariable.UnidirectionalPositions[i + 1, 0])
                    {
        ...
        public void WorkLiftTransfer()
        {
            if (LiftTransferFinish[PilerNums, 0] == true && LiftTransferFinish[PilerNums, 2] == false)
            {
                ...
                {
                    //访问
                    if (Varibles.GlobalVariable.UnidirectionalConveyorStates[PilerNums + 1, 0] == Varibles.State.Off && UnidirectionState[PilerNums + 1, 0] == false)
                    {
                        Varibles.GlobalVariable.UnidirectionalConveyorStates[PilerNums + 1, 0] = Varibles.State.On;
                    ...
                    {
                        this.transform.localPosition = Vector3.MoveTowards(this.transform.localPosition, Varibles.GlobalVariable.UnidirectionalPositions[PilerNums + 1, 0], Speed * Time.deltaTime);
                        if (this.transform.localPosition == Varibles.GlobalVariable.UnidirectionalPositions[PilerNums + 1, 0])
                        {
                    ...
                    if (UnidirectionFinish[PilerNums + 1, 0] == true)
                    {
                        ...
                        if (LiftTransfer1.GetComponent<LiftTransferMove>().Finish2 == true)
                        {
                            ...
                            Tools.Functions.ChangeColor(this.name, Varibles.StorageBinState.NotStored);//修改该货物对应仓位的状态表示颜色
                            this.GetComponent<Varibles.OperatingState>().state = Varibles.CargoState.Exit;
                            Varibles.GlobalVariable.LiftTransferStates[PilerNums] =Varibles. State.Off;
                            Varibles.GlobalVariable.EnterCargosNameList.Remove(this.name);
                        }
                    }
                }
            }
